...
describe('General Usage', () => {
	...
	it('can parse tarballs created by the node-tar module', async () => {
		...
		const fileNames = files.map(f => f.getTrimmedFileName());
		const fileSet = new Set(files);

		for (const subStructure of fileStructures) {
			for (const path of subStructure) {

				const target = files.find(f => f.getTrimmedFileName().endsWith(path) && fileSet.has(f));

				if (!target)
					fail(`path "${path}" not found in files: ${JSON.stringify(fileNames, null, '\t')}`);

				// Force an assertion error so we know which path failed
				if (foundFiles.has(target))
					fail(`found duplicate target "${path}" not found in files: ${JSON.stringify(fileNames, null, '\t')}`);
		...
		if (fileSet.size > 0) {
			const missingFileNames = Array.from(fileSet).map(f => f.getTrimmedFileName());
